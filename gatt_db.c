...
uint8_t GetAccAxesRaw(AxesRaw_t * acceleration_data)
{
 ...
 lis2dw12_flag_data_ready_get(&inertialHandle, &tmp);
  if(tmp)
  {
    memset(data_raw_acceleration.u8bit, 0x00, 3 * sizeof(int16_t));
    /* Read acceleration field data */
    lis2dw12_acceleration_raw_get(&inertialHandle, data_raw_acceleration.i16bit);
    acceleration_data->AXIS_X = (int32_t)lis2dw12_from_fs2_to_mg(data_raw_acceleration.i16bit[0]); 
    acceleration_data->AXIS_Y = (int32_t)lis2dw12_from_fs2_to_mg(data_raw_acceleration.i16bit[1]);
    acceleration_data->AXIS_Z = (int32_t)lis2dw12_from_fs2_to_mg(data_raw_acceleration.i16bit[2]);
  }
...
void GetFreeFallStatus(void) 
{
  ...
  */
 lis2dw12_all_sources_get(&inertialHandle, &all_source);
